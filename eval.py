...
if __name__ == '__main__':
    ...
    parser.add_argument('--data', type=str, default='exp/dtu_scan65/wmask/meshes/00300000.ply')
    ...
    if args.mode == 'mesh':
        ...
        # TriangleMesh with 656182 points and 1312596 triangles.
        data_mesh = o3d.io.read_triangle_mesh(args.data)

        vertices = np.asarray(data_mesh.vertices) # 656182, 3
        triangles = np.asarray(data_mesh.triangles) # 1312596, 3
        tri_vert = vertices[triangles]  # 1312596, 3, 3

        pbar.update(1)
        pbar.set_description('sample pcd from mesh')
        v1 = tri_vert[:,1] - tri_vert[:,0] # 1312572, 3
        v2 = tri_vert[:,2] - tri_vert[:,0]
        l1 = np.linalg.norm(v1, axis=-1, keepdims=True)  # 1312572, 1
        l2 = np.linalg.norm(v2, axis=-1, keepdims=True)
        area2 = np.linalg.norm(np.cross(v1, v2), axis=-1, keepdims=True) # 1312572, 1
        ...
        data_pcd = np.concatenate([vertices, new_pts], axis=0) # 2598797+656182=3254979
    ...
    # BB = array([[-176., -218.,  542.],
    #             [ 193.,  227.,  826.]], dtype=float32)
    # Res = array([[1]], dtype=uint8)

    patch = args.patch_size # 60
    inbound = ((data_down >= BB[:1]-patch) & (data_down < BB[1:]+patch*2)).sum(axis=-1) ==3
    data_in = data_down[inbound] # (2225544, 3)

    data_grid = np.around((data_in - BB[:1]) / Res).astype(np.int32)
    grid_inbound = ((data_grid >= 0) & (data_grid < np.expand_dims(ObsMask.shape, 0))).sum(axis=-1) ==3
    data_grid_in = data_grid[grid_inbound]
    in_obs = ObsMask[data_grid_in[:,0], data_grid_in[:,1], data_grid_in[:,2]].astype(np.bool_)
    data_in_obs = data_in[grid_inbound][in_obs] # (1220492, 3)

    pbar.update(1)
    pbar.set_description('read STL pcd')
    stl_pcd = o3d.io.read_point_cloud(f'{args.dataset_dir}/Points/stl/stl{args.scan:03}_total.ply')
    stl = np.asarray(stl_pcd.points) # (3412248, 3)
    ...
    pbar.close()
    over_all = (mean_d2s + mean_s2d) / 2 # 0.6152068962732264
